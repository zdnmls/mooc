<template>
    <main>
        <my-header/>
        <div class="kj">
            <video ref="video" v-on:click="togglePlay" id="vid" :src="puss" autoplay></video>
            <div class="bottom">
                <!--时间轴-->
                <div class="sjz" v-on:click="seek" v-on:mousemove="move">
                    <div class="sjz-lable" :style="{left:lable.left}">{{lable.time}}</div>
                    <div class="sjz-tiao" :style="{width:left}">
                    </div>
                    <div class="sjz-yuan" :style="{left:left}"></div>
                </div>
                <!--暂停-->
                <div class="bot-left">
                    <div id="bl1" :class="playing?'play':'pause'" v-on:click="togglePlay"></div>
                    <div class="bl2">{{currentLabel}} / {{durationlabel}}</div>
                </div>
                <div class="bot-right">
                    <!--音量-->
                    <div class="br1">
                        <div v-on:click="kg" :class="this.volume===0?'ylicon2':'ylicon'"></div>
                        <div id="volume" v-on:click="setV">
                            <div class="circle2" :style="{width:vwidth}"></div>
                            <div class="circle" :style="{left:vwidth}"></div>
                        </div>
                    </div>
                    <div id="beishu" class="br2">
                        <div class="kuan">倍数</div>
                        <div class="beishu">
                            <h4>倍数播放</h4>
                            <ul v-on:click="videothis">
                                <li class="bs_li" v-on:click="videosudu(2)">2.0x <span></span></li>
                                <li class="bs_li" v-on:click="videosudu(1.75)">1.75x <span></span></li>
                                <li class="bs_li" v-on:click="videosudu(1.5)">1.5x <span></span></li>
                                <li class="bs_li" v-on:click="videosudu(1.25)">1.25x <span></span></li>
                                <li class="bs_li" v-on:click="videosudu(1)">1.0x <span>✔</span></li>
                            </ul>
                        </div>
                    </div>

                    <div class="br2">
                        <div class="kuan">画质</div>
                    </div>
                    <div class="br2">
                        <div class="kuan">线路</div>
                    </div>
                    <div class="br1">设置</div>
                    <div class="br1" v-on:click="result">全屏</div>
                </div>
            </div>
            <div class="you active">
                <div class="you-dian" v-on:click="zj">章节</div>
                <h3>{{list.title}}</h3>
                <ul class="list">
                    <li v-for="(chap,i) in list.chapters" :key="chap.id">
                        <h4>{{chap.title}}</h4>
                        <div v-for="(sec,j) in chap.sections" :key="sec.id" class="video-list" v-on:click="f(sec.src)" :id="sec.id">
                            <div v-on:click="fn">
                                {{(i+1)+'-'+(j+1)}} {{sec.title}}{{sec.times}}<span class="xxzt">正在学...</span>
                            </div>
                        </div>
                    </li>
                </ul>
            </div>
        </div>
    </main>
</template>

<script>
import myHeader from "./header.vue";
export default {
  name: "video1",
  data: function() {
    return {
      playing: true,
      current: 0,
      duration: 0,
      lable: {
        left: 0,
        time: "00:00"
      },
      volume: 1,
      prevV: null,
      puss: "/video/waitclass.mp4",
      vsd: 1, //播放速度,
      results: true,
      list: {}
    };
  },
  components: {
    myHeader
  },
  computed: {
    vwidth: function() {
      return this.volume / 1 * 100 + "%";
    },
    left: function() {
      return this.current / this.duration * 100 + "%";
    },
    currentLabel: function() {
      return this.format(this.current);
    },
    durationlabel: function() {
      return this.format(this.duration);
    }
  },
  methods: {
    togglePlay: function() {
      this.playing = !this.playing;
    },
    //时间的函数变为00:11
    format: function(s) {
      let min = parseInt(s / 60);
      min = min < 10 ? ["0", min].join("") : min;
      let second = parseInt(s % 60);
      second = second < 10 ? ["0", second].join("") : second;
      return [min, ":", second].join("");
    },
    // 进度条的百分比
    seek: function(e) {
      this.current = this.duration * e.offsetX / e.currentTarget.offsetWidth;
      this.$refs.video.currentTime = this.current;
    },
    //音量的开关
    kg: function(e) {
      if (this.prevV) {
        this.volume = this.prevV;
        this.prevV = null;
      } else {
        this.prevV = this.volume;
        this.volume = 0;
      }
    },
    //进度条的lable
    move: function(e) {
      this.lable.left = e.clientX + "px";
      this.lable.time = this.format(
        this.duration * e.clientX / e.currentTarget.offsetWidth
      );
    },
    //音量的百分比
    setV: function(e) {
      this.volume = e.offsetX / e.currentTarget.offsetWidth;
      this.prevV = null;
    },
    videosrc: function(e) {
      this.puss = e;
      this.playing = true;
    },
    //播放倍数
    beishu: function() {
      $(".beishu").toggle();
    },
    videosudu: function(e) {
      this.vsd = e;
    },
    result: function() {
      if (this.results) {
        var de = document.documentElement;
        if (de.requestFullscreen) {
          de.requestFullscreen();
        } else if (de.mozRequestFullScreen) {
          de.mozRequestFullScreen();
        } else if (de.webkitRequestFullScreen) {
          de.webkitRequestFullScreen();
        }
        this.results = false;
      } else {
        var de = document;
        if (de.exitFullscreen) {
          de.exitFullscreen();
        } else if (de.mozCancelFullScreen) {
          de.mozCancelFullScreen();
        } else if (de.webkitCancelFullScreen) {
          de.webkitCancelFullScreen();
        }
        this.results = true;
      }
    },
    videothis: function(e) {
      $(".bs_li")
        .children("span")
        .text("");
      $(e.target)
        .children("span")
        .text("✔");
      let str = $(e.target).text();
      $("#beishu .kuan").text(str.slice(0, str.length - 1));
    },
    //视频播放
    f: function(e) {
      this.puss = e;
    },
    fn: function(e) {
      let dangqian = e.target.lastChild;
      $(".zzx").each((index, v) => {
        $(v).removeClass("zzx");
      });
      $(dangqian).addClass("zzx");
      console.log(this.playing);
    },
    zj: function() {
      let active = $(".you").hasClass("active");
      if (active) {
        $(".you")
          .removeClass("active")
          .animate({ right: "0px" }, 500);
        $("#vid").animate({ "margin-left": "-400px", width: "65%" }, 500);
        $(".you-dian").css("background", "#f01400");
        $(".bottom").animate({ width: "100%" }, 500, function() {
          $(this).css("width", "calc(100% - 400px)");
        });
      } else {
        $(".you")
          .addClass("active")
          .animate({ right: "-400px" }, 500);
        $("#vid").animate({ "margin-left": "0px", width: "75%" }, 500);
        $(".you-dian").css("background", "#363C40");
        $(".bottom").animate({ width: "100%" }, 500);
      }
    }
  },
  watch: {
    playing: function(val) {
      if (val) {
        this.$refs.video.play();
      } else {
        this.$refs.video.pause();
      }
    }
  },
  mounted: function() {
    fetch("/play_data")
      .then(r => r.json())
      .then(v => (this.list = v));
    let v = this.$refs.video;
    v.oncanplay = () => (this.duration = v.duration);
    v.ontimeupdate = () => {
      this.current = v.currentTime;
      v.volume = this.volume;
      v.playbackRate = this.vsd;
    };
    v.onended = () => (this.playing = !this.playing);
  }
};
</script>

<style scoped>
* {
  margin: 0;
  padding: 0;
}

li {
  list-style-type: none;
}

#volume {
  width: 100px;
  height: 3px;
  margin: 0 15px;
  border: 1px solid black;
  background: #999;
  border-radius: 5px;
  position: relative;
}

#volume .circle2 {
  /*width:8px;*/
  height: 3px;
  border-radius: 5px;
  background: #ff1325;
  position: absolute;
}

#volume .circle {
  width: 7px;
  height: 7px;
  border-radius: 50%;
  margin-left: -4px;
  background: #cccccc;
  position: absolute;
  top: -2px;
}

main {
  width: auto;
  height: calc(100vh - 0px);
  min-height: 320px;
  background: #000;
  position: relative;
  display: flex;
  justify-content: center;
  overflow: hidden;
}

.kj {
  width: 100%;
  /*height: auto;*/
  display: flex;
  justify-content: center;
  padding: 50px 0 50px 0;
}

video {
  width: 75%;
  height: calc(100vh - 100px);
  min-height: 220px;
  cursor: pointer;
}

.top {
  width: 100%;
  height: 60px;
  background: #14191e;
  position: absolute;
  top: 0;
  left: 0;
  display: flex;
  justify-content: space-between;
  align-items: center;
  z-index: 999;
  border: 1px solid #000;
  box-sizing: border-box;
}

.top-left {
  width: 60%;
  height: 100%;
  display: flex;
  align-items: center;
  cursor: pointer;
}

.tl1 {
  width: 60px;
  height: 100%;
  font-weight: bold;
  font-family: "宋体";
  line-height: 60px;
  text-align: center;
  font-size: 20px;
  color: #999;
}

.tl2 {
  width: auto;
  max-width: 250px;
  height: 100%;
  line-height: 60px;
  text-align: center;
  font-size: 14px;
  color: #ccc;
  padding: 0 25px;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.tl3 {
  width: auto;
  height: 100%;
  line-height: 60px;
  text-align: center;
  font-size: 12px;
  color: #aaa;
  padding: 0 25px;
}

.top-right {
  width: 30%;
  height: 100%;
  display: flex;
  justify-content: flex-end;
  align-items: center;
}

.tr1 {
  width: 150px;
  height: 100%;
  color: #eeeeee;
  display: flex;
  justify-content: center;
  align-items: center;
}

.gwc {
  width: 100px;
  height: 32px;
  line-height: 32px;
  text-align: center;
  border-radius: 20px;
  font-size: 12px;
  color: #ccc;
  border: 1px solid #999;
}

.tr2 {
  width: 100px;
  height: 100%;
  line-height: 60px;
  text-align: center;
  font-size: 12px;
  color: #ccc;
  padding: 0 20px;
}

.bottom {
  width: 100%;
  height: 40px;
  background: #070c11;
  position: absolute;
  bottom: 0;
  left: 0;
  display: flex;
  justify-content: space-between;
  align-items: center;
  z-index: 999;
}

.sjz {
  width: 100%;
  height: 2px;
  background: #171015;
  /*background: #ff1325;*/
  position: absolute;
  top: 0;
  left: 0;
  z-index: 10;
  cursor: pointer;
}

.sjz:hover {
  width: 100%;
  height: 6px;
  background: #381e0b;
  /*background: #ff1325;*/
  position: absolute;
  top: -4px;
  left: 0;
  z-index: 9999;
  cursor: pointer;
}

.sjz:hover .sjz-tiao {
  height: 6px;
  cursor: pointer;
}

.sjz-lable {
  display: none;
}

.sjz:hover .sjz-lable {
  width: 36px;
  height: 18px;
  padding: 2px;
  border-radius: 3px;
  background: rgba(0, 0, 0, 0.8);
  position: absolute;
  top: -27px;
  left: 10%;
  color: #eeeeee;
  font-size: 12px;
  display: flex;
  justify-content: center;
  align-items: center;
  margin-left: -18px;
}

.sjz-tiao {
  width: 0%;
  height: 2px;
  background: #ff1325;
  position: absolute;
  top: 0;
  left: 0;
  z-index: 9999;
  cursor: pointer;
}

.sjz:hover .sjz-yuan {
  width: 10px;
  height: 10px;
  border-radius: 50%;
  background: #ccc;
  position: absolute;
  top: -2.2px;
  margin-left: -5px;
  z-index: 9999;
}

.bot-left {
  width: auto;
  height: 100%;
  line-height: 40px;
  text-align: center;
  font-size: 14px;
  color: #999;
  display: flex;
  justify-content: flex-start;
  align-items: center;
}

#bl1 {
  width: auto;
  height: 40px;
  padding: 0 25px;
  cursor: pointer;
}

.bl2 {
  width: auto;
  height: 40px;

  padding: 0 25px;
  cursor: default;
}

.bot-right {
  width: auto;
  height: 100%;
  line-height: 40px;
  text-align: center;
  font-size: 12px;
  color: #999;
  display: flex;
  justify-content: flex-start;
  align-items: center;
}

.br1 {
  width: auto;
  height: 40px;
  padding: 0 10px;
  display: flex;
  justify-content: center;
  align-items: center;
  cursor: pointer;
}

.br2 {
  width: auto;
  height: 40px;
  padding: 0 5px;
  display: flex;
  justify-content: center;
  align-items: center;
  cursor: pointer;
}

.kuan {
  width: auto;
  height: 8px;
  padding: 5px;
  display: flex;
  color: #666;
  font-size: 10px;
  border-radius: 2px;
  justify-content: center;
  align-items: center;
  background: #ccc;
  cursor: pointer;
}

.you {
  width: 400px;
  height: 100%;
  background: #14191e;
  position: absolute;
  top: 0;
  right: -400px;
  padding: 60px 35px 60px;
  box-sizing: border-box;
}

.you-dian {
  width: 50px;
  height: 70px;
  background: #363c40;
  display: flex;
  justify-content: center;
  align-items: center;
  font-size: 12px;
  color: #eeeeee;
  position: absolute;
  top: 30%;
  left: -50px;
  /*border:1px solid #000;*/
  box-sizing: border-box;
  cursor: pointer;
}

.you h3 {
  color: #ccc;
  font-size: 16px;
  line-height: 70px;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.list li {
  line-height: 40px;
  color: #666;
  font-size: 14px;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.list li span {
  padding: 15px;
  box-sizing: border-box;
}

.play {
  background: url(data:image/png;base64,)
    no-repeat;
  background-position: center;
  width: 16px;
  height: 16px;
}

.pause {
  left: 12px;
  width: 16px;
  height: 16px;
  cursor: pointer;
  background: url(data:image/png;base64,)
    no-repeat;
  background-position: center;
}

.ylicon {
  width: 35px;
  height: 100%;
  background: url(data:image/png;base64,)
    no-repeat;
  background-position: center;
}

.ylicon2 {
  width: 35px;
  height: 100%;
  background: url(data:image/png;base64,)
    no-repeat;
  background-position: center;
}

.video-list {
  margin-left: 20px;
  color: #8a8a8a;
}

.xxzt {
  opacity: 0;
}

.zzx {
  font-size: 12px;
  color: #00aa00;
  opacity: 1;
}

#beishu {
  position: relative;
  z-index: 30;
}

#beishu:hover .beishu {
  display: block;
}

.beishu {
  display: block;
}

.beishu {
  width: 80px;
  height: 210px;
  border-radius: 5px;
  background: #24272b;
  position: absolute;
  bottom: 39px;
  padding: 0 12px;
  box-sizing: border-box;
  border: 1px solid #555;
  display: none;
}

.beishu h4 {
  font-size: 12px;
  line-height: 30px;
  font-weight: normal;
  border-bottom: 2px solid #666;
}

.bs_li {
  line-height: 34px;
  display: flex;
  justify-content: space-between;
}

.beishu ul {
  margin-top: 6px;
}
</style>